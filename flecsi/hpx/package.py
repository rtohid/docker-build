...
class Flecsi(CMakePackage):
    ...
    depends_on('hpx@master cxxstd=14 build_type=Debug max_cpu_count=128', when='backend=hpx +debug_backend')
    depends_on('hpx@master cxxstd=14 build_type=Release max_cpu_count=128', when='backend=hpx ~debug_backend')
